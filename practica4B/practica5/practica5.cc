...
vector<_vertex3f> Vertices_ant;
vector<_vertex3i> Caras_ant;
...
int pr = 1;
Objeto * PeonBlanco = NULL; 
Objeto * PeonNegro = NULL;
Objeto * PeonMadera = NULL;
Objeto * CocaCola = NULL;

FuenteLuz * fuente_luz_0 = NULL;
FuenteLuz * fuente_luz_1 = NULL;


// Practica 5

// variables para controlar ejes x,y
GLint xc_, yc_, x0_, y0_, yn_, xn_, xm_, ym_;
int estadoRaton = 0;
int xx = 0, yy = 0;
// tamaño bufer selección
const unsigned BUFSIZE = 1000;
// buffer selección
GLuint selectBuf[BUFSIZE];
// variable para saber que objeto a sido seleccionado
GLuint nombre_seleccion = -1;

int matriz_colores[6][3];

int cont_i = 0, cont_j = 0 , cont_k = 0;

int sel_i = 256, sel_j = 256, sel_k = 256;
...
void draw_objects_4() {
    glEnable(GL_LIGHTING);
        ...
        glPushMatrix();
            ...
            PeonBlanco->draw_solid_material_gouroud();
        glPopMatrix();
        glPushMatrix();
            glTranslatef(1.5, 0.0, 0.0);
            glColor3f(0.0, 0.0, 0.0);
            PeonNegro->draw_solid_flat();
        glPopMatrix();
        glPushMatrix();
            glTranslatef(-1.5, 0.0, 0.0);
            //PeonMadera->Madera();
            PeonMadera->draw_solid_flat2();
        glPopMatrix();

        glPushMatrix();
            glTranslatef(0.0, 0.0, 1.5);
            //CocaCola->CocaCola();
            CocaCola->draw_solid_material_gouroud();
        glPopMatrix();
}

void draw_face(_vertex3i cara, vector<_vertex3f> &vertices)  {
    glBegin(GL_TRIANGLES);
        glVertex3f(vertices[cara._0].x, vertices[cara._0].y, vertices[cara._0].z);
        glVertex3f(vertices[cara._1].x, vertices[cara._1].y, vertices[cara._1].z);
        glVertex3f(vertices[cara._2].x, vertices[cara._2].y, vertices[cara._2].z);
    glEnd();
}
...
void draw_solid_color_2 (vector<_vertex3f> &vertices, vector<_vertex3i> &caras, int fig)  {
        bool encontrado;
        for (int i = 0; i < caras.size(); ++i) {
            cont_k++;
            if (cont_k == 256) {
            ...
            if (nombre_seleccion == i && fig_sel == fig )  {
                glColor3f(1.0,1.0, 1.0);  
            } else  {
                glColor3ub(cont_i,cont_j,cont_k);
            }
            draw_face(caras[i], vertices);
    }
}
...
void draw_objects_5()  {
    ...
    glPushMatrix();
        glTranslatef(7, 0, -7);
        glScalef(0.5, 0.5, 0.5);
        draw_solid_color(Vertices_beth, Caras_beth, 7);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(0, 0, -7);
        glScalef(0.5, 0.5, 0.5);
        draw_solid_color(Vertices_beth, Caras_beth, 8);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(-7, 0, -7);
        glScalef(0.5, 0.5, 0.5);
        draw_solid_color(Vertices_beth, Caras_beth, 9);
    glPopMatrix();
...
void draw_objects_6()  {
    ...
    glPushMatrix();
        ...
        draw_solid_color_2(Vertices_beth, Caras_beth, 1);
    glPopMatrix();
    glPushMatrix();
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 2);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(10, 0, 0);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 3);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(10, 0, 10);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 4);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(0, 0, 10);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 5);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(-10, 0, 10);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 6);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(10, 0, -10);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 7);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(0, 0, -10);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 8);
    glPopMatrix();
    glPushMatrix();
        glTranslatef(-10, 0, -10);
        glScalef(0.3, 0.3, 0.3);
        draw_solid_color_2(Vertices_beth, Caras_beth, 9);
    glPopMatrix();
...
leer_datos_ply(vertic, faces, "lata-pcue.ply");
IntToVertexi(faces, Caras);
FloatToVertexf(vertic, Vertices);

// PRACTICA 4
PeonBlanco = new Objeto();
PeonBlanco->create(Vertices, Caras);

PeonNegro = new Objeto();
PeonNegro->create(Vertices, Caras);

PeonMadera = new Objeto();
PeonMadera->create(Vertices, Caras);

CocaCola = new Objeto();
CocaCola->create(Vertices, Caras);
...
hits = glRenderMode(GL_RENDER);
    if(hits!= 0){
        for(int i=0; i<hits; i++) cout<< selectBuf[i]<<" ";
        cout<<"\n";
    }
glMatrixMode(GL_PROJECTION);
glLoadIdentity();
...
void getCamara(GLint &x0_2, GLint &y0_2){

    y0_2=Observer_angle_x;
    x0_2=Observer_angle_y;

}

void setCamara(GLint xn2, GLint yn2){

    Observer_angle_x=yn2;
    Observer_angle_y=xn2;
...
void clickRaton(GLint boton, GLint estado, GLint x, GLint y){
