...
vector<_vertex3f> Vertices_CC;
vector<_vertex3i> Caras_CC;
...
void draw_objects(vector<_vertex3f> vertices, vector<_vertex3i> caras)
{
    if (pr4 == false)  {
        ...
        //CocaCola->draw_solid_flat();
        glPushMatrix();
            glTranslatef(1.5, 0.0, 0.0);
            //Negro->draw_solid_material_gouroud();
        glPopMatrix();
    }
...
{
    if (toupper(Tecla1)=='4') pr4 = true;

    if (pr4 == false) {
        if (toupper(Tecla1)=='Q') { cout << "Adios!" << endl; exit(0);}
...
void initialize(void)
{
    ...
    IntToVertexi(faces, Caras_CC);
    FloatToVertexf(vertic, Vertices_CC);

    CocaCola = new Objeto();
    Negro = new Objeto();
    CocaCola->create(Vertices_CC, Caras_CC);
    Negro->create(Vertices_beth, Caras_beth);
